#!/usr/bin/env python3
"""
Focused Parameter Sweep - Based on initial findings

Key findings from quick sweep:
- min_impulse_score=4 is essential
- min_delta around 20-30 works best
- trailing_stop=6 slightly better than 8
- lvn_ratio and take_profit have minimal impact

This sweep focuses on fine-tuning around these findings.
"""
...
PARAM_RANGES = {
    # Signal parameters - fine-tune around delta=25
    'min_delta': [15, 18, 20, 22, 25, 28, 30, 35],

    # LVN ratio - test a few values
    'max_lvn_ratio': [0.20, 0.25, 0.30],

    # Impulse - score=4 is essential, vary size
    'min_impulse_size': [15.0, 20.0, 25.0],
    'min_impulse_score': [4],  # Fixed at 4

    # Trade management - fine-tune
...
def main():
    output_file = 'sweep_focused.csv'
    combinations = generate_combinations(PARAM_RANGES)

    print(f"Focused Parameter Sweep")
    ...
    with open(output_file, 'w', newline='') as f:
        ...
        for i, params in enumerate(combinations):
            ...
            print(f"\r[{i+1}/{len(combinations)}] d={params['min_delta']} lvn={params['max_lvn_ratio']} "
                  f"tp={params['take_profit']} tr={params['trailing_stop']} hunt={params['max_hunting_bars']} "
    ...
    if profitable:
        ...
        for i, r in enumerate(profitable[:5], 1):
            print(f"  {i}. Sharpe={float(r['sharpe_ratio']):.2f} PF={float(r['profit_factor']):.2f} "
                  f"P&L={float(r['total_pnl']):+.1f} Trades={r['total_trades']} "
                  f"| d={r['min_delta']} tr={r['trailing_stop']} tp={r['take_profit']}")
