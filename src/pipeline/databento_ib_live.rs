...
use hitthebid::tradovate::TradovateExecutor;
...
/// Live trading with Databento data + Tradovate execution
pub async fn run_tradovate_mode(
    api_key: String,
    contract_symbol: String,
    config: LiveConfig,
    sm_config: StateMachineConfig,
    mut executor: TradovateExecutor,
    trade_log: std::path::PathBuf,
) -> Result<()> {
    use std::io::Write;
    use hitthebid::tradovate::{Direction as TvDirection, TradeAction as TvTradeAction};
    ...
    println!();
    println!("📊 Data Source: Databento GLBX.MDP3");
    println!("🏦 Execution: Tradovate API");
    println!("📝 Trade Log: {:?}", trade_log);
    println!("📈 Contract: {}", contract_symbol);
    println!("⏰ Trading Hours: {:02}:{:02} - {:02}:{:02} ET",
        config.start_hour, config.start_minute,
                ...
                if let Some(action) = trader.process_bar(&bar) {
                    ...
                    match action {
                        TradeAction::Enter { direction, price: entry_price, stop, target, contracts } => {
                            ...
                            // Convert to Tradovate types and execute
                            let tv_direction = if matches!(direction, Direction::Long) {
                                TvDirection::Long
                            } else {
                                TvDirection::Short
                            };

                            let tv_action = TvTradeAction::Enter {
                            ...
                            let executed = match executor.execute(tv_action).await {
                                Ok(_) => {
                                    println!("  ✅ ORDER EXECUTED via Tradovate | {}", now);
                                    true
                                }
                                Err(e) => {
                                    error!("  ❌ EXECUTION FAILED: {} | {}", e, now);
                        ...
                        TradeAction::Exit { direction, price: exit_price, pnl_points, reason } => {
                            ...
                            let tv_action = TvTradeAction::Exit {
                                direction: tv_direction,
                                price: exit_price,
                                pnl_points,
                                reason: reason.clone(),
                        ...
                        TradeAction::UpdateStop { new_stop } => {
                            debug!("Updating stop to {:.2}", new_stop);
                            let tv_action = TvTradeAction::UpdateStop { new_stop };
                            if let Err(e) = executor.execute(tv_action).await {
                                warn!("Failed to update stop: {}", e);
                            }
                        }
                        TradeAction::FlattenAll { reason } => {
                            println!("⚠️  FLATTEN ALL: {}", reason);
                            let tv_action = TvTradeAction::FlattenAll { reason: reason.clone() };
                            if let Err(e) = executor.execute(tv_action).await {
                                error!("Failed to flatten: {}", e);
                            }
                            break;
