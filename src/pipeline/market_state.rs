...
mod tests {
    ...
    fn make_bar(close: f64, high: f64, low: f64, delta: i64, volume: u64) -> Bar {
        Bar {
            timestamp: Utc::now(),
            open: close,
            high,
    ...
    fn test_balanced_market() {
        // Create bars that oscillate around 100.0
        let bars = vec![
            make_bar(99.0, 101.0, 98.0, 10, 1000),
            make_bar(101.0, 102.0, 99.0, -5, 1000),
            make_bar(99.5, 101.0, 98.5, 8, 1000),
            make_bar(100.5, 102.0, 99.0, -3, 1000),
            make_bar(99.0, 100.5, 98.0, 5, 1000),
        ];
        ...
        let result = detect_market_state(&bars, 4, &config);
        assert_eq!(result.state, MarketState::Balanced);
    }

    #[test]
    fn test_imbalanced_market_delta() {
        // Create bars with strong directional delta
        let bars = vec![
            make_bar(100.0, 101.0, 99.0, 200, 1000),
            make_bar(101.0, 102.0, 100.0, 200, 1000),
            make_bar(102.0, 103.0, 101.0, 200, 1000),
            make_bar(103.0, 104.0, 102.0, 200, 1000),
            make_bar(104.0, 105.0, 103.0, 200, 1000),
        ];

        let config = MarketStateConfig {
            lookback_bars: 5,
            delta_accumulation_threshold: 500,
            ..Default::default()
        };

        let result = detect_market_state(&bars, 4, &config);
        assert_eq!(result.state, MarketState::Imbalanced);
    }
}
